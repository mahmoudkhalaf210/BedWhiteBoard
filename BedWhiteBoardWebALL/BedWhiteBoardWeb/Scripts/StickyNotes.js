...
function General_ValdateText(txt) {
...
function FireToster_success(txt) {
    TosterOptions();
    toastr.success(txt);
...
function TosterOptions_dashboards() {
    toastr.options = {
        //"closeButton": true,
        "debug": false,
        "newestOnTop": true,
...
function FireToster_Error(txt) {
    TosterOptions();
    toastr.error(txt);
}


function FireToster_successt(txt, Title) {
    TosterOptions();
    toastr.success(txt, Title);

}

function FireToster_waringt(txt, Title) {
    TosterOptions();
...
function FireToster_Errort(txt, Title) {
    TosterOptions();
...
function FireTosterdash_success(title, txt) {
    TosterOptions_dashboards();
    return toastr.success('<span>' + title + '</span>' + txt, "Loading...");
...
function StickyNotes(Patient_id, Episdoe_type, episode_key, patientname, HMRN, isvedio, vedioPatientData) {
    ...
    STickynotes.rmoveSticknote = function (item) {
        ...
        vm.AllStickNotes.remove(function (player) {
    ...
    STickynotes.DeleteSticknote = function () {
        ...
        //var taoster = FireTosterdash_success(('AddSticknotes'), Patient_ID + ' ' + PatientName);
        ...
        $.post(url, dataToPass).done(function (output) {
            ...
            if (output == 'true') {
                ...
                //FireToster_successt(('AddingStrikyNoteSucesss'), PID);
            } else {
                //FireToster_Errort(('ErrorAddStickyNotes'), PID);
                console.log(output);
            }
    ...
    STickynotes.addSticknote = function () {
        ...
        var taoster = FireTosterdash_success(('AddSticknotes'), STickynotes.HMRN() + ' ' + STickynotes.patientname());
...
function dishargePatient() {
    ...
    dishargePatient.loaddata = function (Episode_Key, PId, PatientName, clinicKey, Cons_SysKey, OUTPATREGSSYS) {
        ...
        $.getJSON("/BedWhiteBoardWeb/BedManagementApiActions/WaitingPatients/GetDischareStatus?hospid=" + AllPublicGeneralData.Hospid, function (allData) {
            ...
            var allaatas = $.map(allData, function (ConsIn) { return new rowModel(ConsIn, ConsultationsIn); })
    ...
    dishargePatient.DischarePatientnow = function () {
        if (!dishargePatient.selected()) {
            FireToster_Errort(('discharetstus'), '')
            return;
        }
        ...
        $.getJSON("/BedWhiteBoardWeb/BedManagementApiActions/Nursestations/CheckForDischargeDoc?patId=" + pId + "&epskey=" + episodeKey + "&DocCode=" + doccode + "&orderkey=" + episodeKey, function (allData) {
            if (allData > 0) {
                var Actiontext = ('FinalDischargeer');
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == "0" | data == "1") {
                            FireToster_successt(('sucessFinscharge'), PID)
                            if (dishargePatient.OUTPATREGSSYS() != undefined) {
                                Dashborad_GetNotificationToOUTPATS(dishargePatient.OUTPATREGSSYS(), Actiontext, ('sucessFinscharge') + ' ' + PID, 1);
                            }
                            ...
                            toastr.clear(taoster);
                            FireToster_Errort(('ErrorFinscharge'), PID)
                            console.log(output);
                        }
                    }
                    ,
...
function LogoutSesionViewModel() {
    ...
    LogoutSesion.logoutone = function (item) {
        ...
        bootbox.prompt(('Requestpincode'), function (result) {
            ...
            if (result.trim() == AllPublicSysparameters.PinCode) {
                ...
                FireToster_Errort(('errorpincode'), '');
...
function MessagesViewModel() {
    ...
    MessagesView.searchEnter = function () {
        ...
        var taoster = FireTosterdash_success(('searchfor'), MessagesView.searchtoText());
    ...
    MessagesView.AddSignature = function () {
        ...
        var taoster = FireTosterdash_success(('addSignure'), '');
        ...
        $.getJSON(url, function (data) {
            FireToster_successt(('generalsavd'), '');
            toastr.clear(taoster);
            MessagesView.loadSignure(1);
        }).fail(function (output) {
            FireToster_Errort(('errorinsave'), '');
            console.log(output);
            toastr.clear(taoster);
        });
    }
    ...
    MessagesView.loadMsg = function (loadtype) {
        ...
        //if (taoster_1 == null) {
        //    taoster_1 = FireTosterdash_success(('Messages'), '');
        //}
        $.getJSON(url, function (AllMessages) {
    ...
    MessagesView.sentMessage = function () {
        ...
        if (body.trim().length < 5 | title.length < 5 | MessagesView.selectList().length == 0) {
            if (body.trim().length < 5 | body == "undefined" | body == undefined) {
                FireToster_Errort(('missingmsgbody'), '');
                return;
            }
            if (title.trim().length < 5 | title == "undefined" | title == undefined) {
                FireToster_Errort(('missingmsgtitle'), '');
                return;
            }
            if (MessagesView.selectList().length == 0) {
                FireToster_Errort(('missingmsglists'), '');
                return;
            }
        ...
        var taoster = FireTosterdash_success(('sendmessages'), title);
        $.post(url, dataToPass).done(function (output) {
            toastr.clear(taoster);
            MessagesView.LoadOffileAttachs([]);
            if (output == 'error') {
                FireToster_Errort(('errorsendmsg'), title);
            } else {
                FireToster_successt(('donesendmsg'), title);
                var message = "";
    ...
    MessagesView.ReplayMessage = function (type) {
        ...
        var taoster = FireTosterdash_success(('sendmessages'), newtitle);
        $.post(url, dataToPass).done(function (output) {
            toastr.clear(taoster);
            console.log(output);
            MessagesView.LoadOffileAttachs([]);
            if (output == 'error') {
                FireToster_Errort(('errorsendmsg'), newtitle);
            } else {

                var MsgKey = output.split(",")[0].replace("-", "");
                output = output.replace("-" + MsgKey + ",", "")
                //alert(MsgKey);
                FireToster_successt(('donesendmsg'), newtitle);
