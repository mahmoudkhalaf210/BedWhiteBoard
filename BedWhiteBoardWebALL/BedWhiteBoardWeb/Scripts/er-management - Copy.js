...
//'bedwhiteboardweb/erwhiteboard.aspx'
//ChatMessage('tomodule', '3177', '[{"MessageHeader":"ER PATIENT ARRIVED", "MessageBody":[{"sys_key":"111","patient_id":"123","alert_name":"123","add_or_remove":"add","alert_type":"123","tooltip":"123","icon":"123","color":"123"}]}]');
    ...
    if (e.itemData.id == "0") {
        if (selectedWP.emr_status() == "0" || selectedWP.emr_status() == "10" || selectedWP.emr_status() == null) {
            if (e.itemData.text.startsWith("Triage")) {
                ...
                if (fromWindows != 1) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            //childwindow = window.parent.AddNewTabwon(url + '&Distrct=' + localStorage.getItem('vvDistrictId') + '&hospitalid=' + localStorage.getItem('vvHospID'));

                            //childwindow = window.open(url, "mywindow", "location=0,status=0,scrollbars=0,menubar=0,directories=0,resizable=1,scrollbars=1,width=1200,height=800");
				...
				'&pname=' + selectedWP.patengname() +'&skey=' + sKey)},300);
            }
...
var mnuTrtmsOnBedItemClicked = function (e) {
    ...
    if (e.itemData.id != "0") {
        ...
        if (fromWindows == 1) {
            ...
            url = "../AdmissionRequestUI/AdmissionRequest.html?lang=" + lang + "&hospitalid=" + hospitalid + "&userkey=" + uId + "&bookingkey=" + sKey + "&iser=1&docid=" + docId + "&patientid=" + pId + "&isedit=0&regsnumber=0";
        ...
        if ($($('#nsWp' + self.sys_key()).parents(3)[2]).hasClass('panelfullwidthSel')) {
            ...
            $("#myTrt" + openedNS).change(function () {
                if ($(this).prop('checked') === true) {
                    ...
                    rows.find("td:nth-child(7)").filter(":containsIN('" + uId + "')").parent().show();
                    rows.find("td:nth-child(7)").filter(":containsIN('" + uId + "')").parent().prev().show();
                }
                else
    ...
    if (notOnBed.seenbyerdoctor_time() == null || notOnBed.seenbyerdoctor_time() == '') {
        $.each(mnuTrtmsNotOnBed, function (key, value) {
            if (value != undefined) {
                ...
                //if (mnuTrtmsNotOnBed[key].text.startsWith("Patient Medical") && mnuTrtmsNotOnBed[key].iconSrc == 'images/Patient-Medical-Record.png')
                //    mnuTrtmsNotOnBed[key].iconSrc = 'images/Request-for-Admission.png';
            }
        });
    }
    else
    {
        $.each(mnuTrtmsNotOnBed, function (key, value) {
            if (value != undefined) {
                ...
                //if (mnuTrtmsNotOnBed[key].text.startsWith("Patient Medical") && mnuTrtmsNotOnBed[key].iconSrc == 'images/Request-for-Admission.png')
                //    mnuTrtmsNotOnBed[key].iconSrc = 'images/Patient-Medical-Record.png';
            }
        });
    }
                ...
                buttons: {
                    confirm: function () {
                        ...
                        $.ajax({
                            ...
                            contentType: "application/json; charset=utf-8",
                            dataType: "json",
                            url: "BedManagementApiActions/WaitingPatients/DischargePatient?ek=" + episodeKey + "&pId=" + pId + "&sKey=" + sKey + "&dStatus=" + statId + "&uId=" + uId + "&hospId=" + hospitalid,
                            success: function (data) {
                                //console.log(data);
                                if (data == "0") {
...
var WaitingPatient = function (Wp) {
    ...
    //self.mnuWPsVisible = ko.observable(false);
            ...
            ko.utils.arrayForEach(vm.nursestations(), function (ns) {
                ...
                if (foundRes == false) {
                    ko.utils.arrayForEach(ns.roomsAndBeds(), function (room) {
                        ko.utils.arrayForEach(room.beds(), function (bed) {
                            if (bed != undefined) {
                                //console.log(bed.patengname().toString().toLowerCase().trim() + "   " + searchtext.toLowerCase().trim());
...
//function init() {
//    var sorter = tsorter.create('tblWP762');
//}
...
function DoctorSearch()
{
	...
	{
		if (jsonErDoctors[i].doctor_name.toLowerCase().indexOf($("#docsearch").val())>=0)
		{
